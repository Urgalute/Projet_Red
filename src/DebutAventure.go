...
func (p *Player) SuiteForet() {
	...
	switch input {
	case "1":
		fmt.Println(p.name, "\033[1m - Allez, c'est parti !")
		fmt.Println(" ")
		time.Sleep(500 * time.Millisecond)
		...
		case "D":
			...
			if résultat >= 17 {
				...
				time.Sleep(500 * time.Millisecond)
				fmt.Println("téméraires que vous, et qu'ils feront le tour.\033[0m")
				time.Sleep(500 * time.Millisecond)
				fmt.Println("  ")
				p.SuiteChemin()
				return
				...
				time.Sleep(500 * time.Millisecond)
				fmt.Println("Mais vous arrivez tout de même sain et sauf de l'autre côté\033[0m")
				time.Sleep(500 * time.Millisecond)
				p.SuiteChemin()
				return
				...
				time.Sleep(500 * time.Millisecond)
				fmt.Println("Vous arrivez finalement de l'autre côté sans autre frayeur, mais avec une belle plaie.\033[0m")
				time.Sleep(500 * time.Millisecond)
				p.SuiteChemin()
				return
			} else if résultat < 5 {
...
func (p *Player) Massure() {
	...
	case "1":
		...
		time.Sleep(500 * time.Millisecond)
		fmt.Println(p.name, " - Bonjour, je suis un peu perdu, sauriez-vous où se situe la ville la plus proche ?")
		time.Sleep(500 * time.Millisecond)
		fmt.Println("\033[93mVieil Homme\033[0m - L'île la plus proche ? ")
		time.Sleep(500 * time.Millisecond)
		fmt.Println(p.name, " - Non, non, LA VILLE la plus proche !")
		time.Sleep(500 * time.Millisecond)
		fmt.Println("\033[93mVieil Homme\033[0m - Ah ! La ville, et bien vous avez la ville d'Alyoli si vous continuez plus au sud")
		time.Sleep(500 * time.Millisecond)
		fmt.Println(p.name, " - Merci, quel serait le chemin le plus rapide ?")
		time.Sleep(500 * time.Millisecond)

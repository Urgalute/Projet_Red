...
func (p *Player) Auberge() {
	...
	case "2":
		if !p.CheckItem("Clé du Donjon Obscur") {
			time.Sleep(500 * time.Millisecond)
			fmt.Println(p.name, "- Je souhaiterai avoir des informations sur le donjon pas")
			time.Sleep(500 * time.Millisecond)
			...
			fmt.Println(" ")
			time.Sleep(500 * time.Millisecond)
			fmt.Println("Serveuse - Allez voir l'aubergiste pour ça.")
			time.Sleep(500 * time.Millisecond)
			fmt.Println("\033[1mElle s'éloigne et reprend son travail avec d'autres clients.")
			time.Sleep(500 * time.Millisecond)
			fmt.Println("Vous regardez l'aubergiste qui à l'air extrêmement préoccupé,")
			...
			time.Sleep(500 * time.Millisecond)
			fmt.Println(p.name, "- Bonjour, votre serveuse m'a indiquée que vous auriez des renseignements !")
			time.Sleep(500 * time.Millisecond)
			fmt.Println("sur le donjon pas loin ?")
			fmt.Println("Aubergiste - Oh, oui, c'était mon manoir il y a encore quelques mois ...")
			time.Sleep(500 * time.Millisecond)
			fmt.Println(p.name, "- Que s'est-il passé ?")
			time.Sleep(500 * time.Millisecond)
			fmt.Println("Aubergiste - Un monstre énorme est arrivé et à commencer à ravager les environs.")
			time.Sleep(500 * time.Millisecond)
			fmt.Println(p.name, "- Je vois.")
			time.Sleep(500 * time.Millisecond)
			fmt.Println("Aubergiste - Il a investi mon manoir, et il est impossible de l'en déloger, il est trop puissant.")
			time.Sleep(500 * time.Millisecond)
			fmt.Println(p.name, "- Que diriez-vous que je m'en charge ?")
			time.Sleep(500 * time.Millisecond)
			fmt.Println("Aubergiste - Vous feriez ça ? Très bien, je vais poser mes espoirs en vous. Voici la clé de l'entrée.")
			time.Sleep(500 * time.Millisecond)
			fmt.Println(p.name, "- Merci de votre confiance, je m'y rend dès que je me suis équipé et que je suis prêt.")
			time.Sleep(500 * time.Millisecond)
			fmt.Println("Aubergiste - Merci ... à bientôt j'espère ...")
			time.Sleep(500 * time.Millisecond)
			fmt.Println(" ")
			...
			fmt.Println("Aubergiste - Alors qu'en est-il de mon problème de donjon ?")
			time.Sleep(500 * time.Millisecond)
			fmt.Println(p.name, "- j'y travaille !")
			time.Sleep(500 * time.Millisecond)
			fmt.Println("Aubergiste - Revenez me voir quand vous aurez du nouveau !")
			time.Sleep(500 * time.Millisecond)
			fmt.Println(p.name, "- Pas de problème !")
			time.Sleep(500 * time.Millisecond)
			p.MainMenu()
		}
	}
